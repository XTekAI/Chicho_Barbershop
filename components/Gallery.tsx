                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }} className="gallery-grid">
                    {galleryItems.map((item, i) => (
                        <div
                            key={item.src}
                            className="reveal"
                            style={{ position: 'relative', borderRadius: '12px', overflow: 'hidden', aspectRatio: '1 / 1', border: '1px solid #2a2a2a', cursor: 'pointer', transition: 'border-color 0.3s ease, transform 0.3s ease', transitionDelay: `${i * 0.06}s` }}
                            onMouseEnter={(e) => { const el = e.currentTarget as HTMLElement; el.style.borderColor = '#ADFF2F'; el.style.transform = 'scale(1.02)'; }}
                            onMouseLeave={(e) => { const el = e.currentTarget as HTMLElement; el.style.borderColor = '#2a2a2a'; el.style.transform = 'scale(1)'; }}
                        >
                            <Image src={item.src} alt={item.alt} fill sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" style={{ objectFit: 'cover' }} />
