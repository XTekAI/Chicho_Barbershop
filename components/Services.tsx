...
function ServiceCard({ name, desc, img, alt, href, external, icon, index }: {
    ...
    return (
        <a
            ...
            style={{ overflow: 'hidden', transition: 'border-color 0.3s ease, transform 0.3s ease', transitionDelay: `${index * 0.07}s`, cursor: 'pointer', display: 'flex', flexDirection: 'column', textDecoration: 'none' }}
            onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.borderColor = '#ADFF2F'; (e.currentTarget as HTMLElement).style.transform = 'translateY(-4px)'; }}
        ...
        >
            <div style={{ position: 'relative', height: '200px', overflow: 'hidden' }}>
                <Image src={img} alt={alt} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" style={{ objectFit: 'cover' }} />
...
export default function Services() {
    ...
    const services = servicesMeta.map((meta, i) => ({ ...meta, ...t.items[i] }));
    ...
    return (
        <section id="services" ref={sectionRef} className="section-padding" style={{ background: 'var(--charcoal)' }}>
            <div className="container-max">
                ...
                <div className="services-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.25rem', maxWidth: '900px', margin: '0 auto 1.25rem' }}>
                    {services.slice(0, 3).map((s, i) => <ServiceCard key={i} {...s} index={i} />)}
                </div>

                <div className="services-grid-2" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1.25rem', maxWidth: '600px', margin: '0 auto' }}>
                    {services.slice(3).map((s, i) => <ServiceCard key={i} {...s} index={i + 3} />)}
                </div>
            </div>
